...
void create_chart_with_data(lv_obj_t *chart, blower_test_value_t *b_vals) {
	...
	if (!data_points || data_points_count == 0) {
		ESP_LOGW(tag,
				...
				__FUNCTION__, __LINE__, (void* )data_points, data_points_count);
	}
	...
	memset(chart_data_burnin, DEF_OFFSET_VAL, sizeof(chart_data_burnin));
	memset(chart_data_prev_val, DEF_OFFSET_VAL, sizeof(chart_data_prev_val));
...
//	lv_coord_t maj = 0;
	lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_Y, 5, 1, num_ticks_y, 1,
	true, 30);
...
//	lv_coord_t *y_ser = lv_chart_get_y_array(chart, series);
	...
	for (size_t i = 0; i < 10; i++) {
		if (i < 2) {
			...
			lv_chart_set_next_value(chart, ser1, (int16_t) LV_CHART_POINT_NONE);
			lv_chart_set_next_value(chart, ser2, (int16_t) data_points[i - 2]);
		}
		else {
	...
	ESP_LOGD(tag, "[%s, %d]Finished adding points", __FUNCTION__, __LINE__);
